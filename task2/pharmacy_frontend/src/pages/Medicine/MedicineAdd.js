...
const MedicineAdd = () => {
  ...
  const submit = async () => {
    if (validate()) {
      ...
      const data = {
        ...
        medicine_type_id: values.medicine_type_id
      };

      postU('medicines', data)
        .then((resp) => {
  ...
  return (
    <>
      <Helmet>
        <title>Create new medicine type</title>
      </Helmet>
      <Box sx={{ pt: 2 }}>
      ...
      <Box sx={{ backgroundColor: 'background.default', minHeight: '100%' }}>
        <Container maxWidth={false}>
          <Box sx={{ pt: 2 }}>
            <form>
              <Card>
                <CardHeader title="Добавление нового типа лекарств" />
                <Divider />
...
<FormControl fullWidth sx={{mt: 2, mb: 1}}>
                                        <InputLabel id="medicine_type_id">
                                        Medicine Type
                                        </InputLabel>
                                        <Select
